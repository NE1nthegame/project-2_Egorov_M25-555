...
from .core import (
    create_table, drop_table, list_tables, get_table_columns, 
    get_table_schema, insert, select, update, delete, format_table_output, get_table_info
)
...
def _handle_insert(metadata, args):
    ...
    if len(args) < 4 or args[1].lower() != 'into' or args[3].lower() != 'values':
        print("Ошибка: Неверный формат команды. Использование: insert into <имя_таблицы> values (<значение1>, <значение2>, ...)")
        return
    ...
    try:
        ...
        table_data, new_id = insert(metadata, table_name, values)
...
def _handle_select(metadata, args):
    ...
    if len(args) < 3 or args[1].lower() != 'from':
        print("Ошибка: Неверный формат команды. Использование: select from <имя_таблицы> [where <условие>]")
        return
...
def _handle_update(metadata, args):
    ...
    if len(args) < 7 or args[2].lower() != 'set' or 'where' not in [arg.lower() for arg in args]:
        print("Ошибка: Неверный формат команды. Использование: update <имя_таблицы> set <столбец>=<значение> where <условие>")
        return
    ...
    if set_index == -1 or where_index == -1:
        print("Ошибка: Неверный формат команды. Должны присутствовать SET и WHERE.")
        return
    ...
    try:
        ...
        table_data, updated_count = update(table_name, set_clause, where_clause)
...
def _handle_delete(metadata, args):
    ...
    if len(args) < 5 or args[1].lower() != 'from' or args[3].lower() != 'where':
        print("Ошибка: Неверный формат команды. Использование: delete from <имя_таблицы> where <условие>")
        return
    ...
    try:
        ...
        table_data, deleted_count = delete(table_name, where_clause)
...
def _handle_info(metadata, args):
    ...
    if len(args) != 2:
        print("Ошибка: Неверное количество аргументов. Использование: info <имя_таблицы>")
        return
